...
def analyze_log_file(log_file):
    """Extract lines containing the word 'Exception'."""
    Exception_lines = []
    lines = []
    try:
        with open(log_file, 'r') as f:
            ...
            for i, line in enumerate(lines):
                if "Exception" in line:
                    Exception_lines.append((i, line.strip()))
    except Exception as e:
        messagebox.showerror("Error", f"An error occurred while reading the file: {e}")
        return [], []
    return Exception_lines, lines

def display_results(Exception_lines, lines, log_file):
    """Display the lines containing 'Exception' and allow graph selection."""
    results_window = tk.Toplevel()
    results_window.title("Exception Lines & Graph")
    ...
    def highlight_line(line_num):
        """Highlight the selected Exception line in the log content."""
    ...
    # Exception lines display
    Exception_frame = tk.Frame(results_window, bg="white", relief=tk.RAISED, borderwidth=2)
    Exception_frame.pack(side="right", fill="both", expand=True, padx=10, pady=10)

    Exception_canvas = tk.Canvas(Exception_frame, bg="white")
    Exception_scrollbar = tk.Scrollbar(Exception_frame, orient="vertical", command=Exception_canvas.yview, bg="#80CFA9")
    Exception_scrollable_frame = tk.Frame(Exception_canvas, bg="white")

    Exception_scrollable_frame.bind("<Configure>", lambda e: Exception_canvas.configure(scrollregion=Exception_canvas.bbox("all")))
    Exception_canvas.create_window((0, 0), window=Exception_scrollable_frame, anchor="nw")
    Exception_canvas.configure(yscrollcommand=Exception_scrollbar.set)

    Exception_canvas.pack(side="left", fill="both", expand=True)
    Exception_scrollbar.pack(side="right", fill="y")

    total_lines = len(lines)
    total_Exceptions = len(Exception_lines)
    Exception_percentage = (total_Exceptions / total_lines * 100) if total_lines > 0 else 0

    # Labels for statistics at the top left corner of Exception lines
    stats_frame = tk.Frame(Exception_scrollable_frame, bg="lightblue")
    ...
    tk.Label(stats_frame, text=f"Total Exception Lines: {total_Exceptions}", font=("Times New Roman", 14, "italic"), bg="lightblue", anchor="w").pack(anchor="w", padx=5, pady=2)
    tk.Label(stats_frame, text=f"Percentage of Exception Lines: {Exception_percentage:.2f}%", font=("Times New Roman", 14, "italic"), bg="lightblue", anchor="w").pack(anchor="w", padx=5, pady=2)

    for index, message in Exception_lines:
        def on_click(idx=index):
            highlight_line(idx)

        label = tk.Label(Exception_scrollable_frame, text=f"Line {index + 1}: {message}", font=("Times New Roman", 12), anchor="w", fg="blue", cursor="hand2", bg="white")
        label.pack(anchor="w", padx=10, pady=2)
        label.bind("<Button-1>", lambda event, idx=index: on_click(idx))

    if not Exception_lines:
        tk.Label(Exception_scrollable_frame, text="No lines containing 'Exception' found in the log file.", font=("Times New Roman", 14), bg="white").pack(pady=10)

    # Graph plotting functions
    def show_pie_chart():
        """Plot and display a pie chart."""
        fig, ax = plt.subplots(figsize=(6, 4))
        labels = ['Exception Lines', 'Other Lines']
        sizes = [total_Exceptions, total_lines - total_Exceptions]
    ...
    def show_line_chart():
        ...
        fig, ax = plt.subplots(figsize=(6, 4))
        x = ['Exception Lines', 'Other Lines']
        y = [total_Exceptions, total_lines - total_Exceptions]
        ax.plot(x, y, marker='o', linestyle='-', color='yellow')
    ...
    def show_bar_chart():
        ...
        fig, ax = plt.subplots(figsize=(6, 4))
        categories = ['Exception Lines', 'Other Lines']
        counts = [total_Exceptions, total_lines - total_Exceptions]
        ax.bar(categories, counts, color=['yellow', 'lightgreen'])
    ...
    def generate_report():
        ...
        report_data = {
            "Category": ["Exception Lines", "Other Lines", "Total Lines"],
            "Count": [total_Exceptions, total_lines - total_Exceptions, total_lines],
        }
        df = pd.DataFrame(report_data)

        try:
            report_path = os.path.join(os.path.dirname(log_file), "Exception_report.xlsx")
...
def process_log_file(log_file):
    """Process the log file and display Exception lines."""
    Exception_lines, lines = analyze_log_file(log_file)
    if Exception_lines or lines:
        display_results(Exception_lines, lines, log_file)
